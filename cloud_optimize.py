...
# Writing bounds #
bounds= numpy.asarray([[0,0,25],[150,150,150]])
...
def optimize(cloud, edges):
  ...
  while (i < 100):
    ...
    originalPos = randomMove(cloud, worstPT, 50, bounds)
...
def randomMove(cloud, index, radius, bounds):
  ...
  while t < 100:
    ...
    failure = False
    for i in range(len(cloud)):
      if i != index and dist(mv_cloud,cloud[i]) < 15.0 or not off_bounds(mv_cloud, bounds):
        #cloud[i] = mv_cloud
        failure = True
